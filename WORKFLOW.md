...
> dans mes projets via submodule. Suite au découplage Brain ↔ DB-LLM
...
- `chore(projects): mise à jour état Brain phase 2`
- `refactor: ...` (réorganisation de structure)

Scopes utiles ici : `skill`, `playbook`, `memory`, `gotchas`,
`decisions`, `lessons`, `projects`, `brain`, `global`.
...
Quand une phase d'un projet se termine (ex: Brain Phase 1 → Phase 2) :
