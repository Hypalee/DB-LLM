...
Pour un setup avec un repo `claude-config` global et un projet `<projet>` :

```
claude-config/                       # Repo de configuration partagé
├── CLAUDE.md                        # Profil dev, stack, conventions universelles
├── projects/
│   └── <projet>.md                     # Contexte produit <projet> (cible, promesse, archi)
├── memory/
...
**Claude.ai** : tu lui colles le contenu de `claude-config/CLAUDE.md` + `claude-config/projects/<projet>.md` au début d'une session significative. Il a alors le contexte produit complet, sans avoir à tout réexpliquer.

**Claude Code** : il lit automatiquement le `CLAUDE.md` à la racine du repo <projet> à chaque session. Ce `CLAUDE.md` contient `@claude-config/CLAUDE.md` et `@claude-config/projects/<projet>.md`, ce qui résout l'import via le submodule. Il a donc accès à tout le contexte sans intervention.
...
```md
## 2026-04-25 14:30 — Session audit initial <projet>
...
**Pièges rencontrés** :
- Le from Resend est hardcodé à noreply@<domaine.tld>, domaine à vérifier
...
**Exemple concret** : nous avons utilisé ce pattern dans la conversation <projet> pour faire transitionner du cadrage produit (décisions sur cible, promesse, pricing, archi LLM) vers l'audit technique fait par Claude Code via les PROMPT_1, PROMPT_2 et PROMPT_3.
...
**Effort estimé** : 1-2 jours pour une v0 fonctionnelle. À envisager après la bêta de <projet>.

### 7.2 Webhooks GitHub → Claude.ai

**Idée** : configurer un webhook qui notifie Claude.ai quand un commit important est pushé sur <projet>, avec un résumé. Claude.ai peut alors analyser et commenter de façon proactive.
...
*Chapitre rédigé en avril 2026, dans le contexte du projet <projet>. À mettre à jour quand l'écosystème Anthropic évolue (Claude Design étant encore en research preview, sortie publique récente).*
