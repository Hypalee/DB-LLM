...
> en place avec `claude-config` importé dans <projet> via submodule.
> Le couplage entre les deux repos a été abandonné le 2026-04-26
> (cf. `memory/decisions.md` → "Découplage <projet> ↔ DB-LLM"). La
...
tes projets (<projet>, projet suivant…) sans avoir à les reconfigurer
à chaque repo.
...
- Chaque repo doit avoir son propre `.mcp.json`. Quand tu crées
  <projet> comme repo séparé, il faudra y mettre un `.mcp.json` aussi.
  Convention : copier celui de `claude-config` comme base, ajuster.

### Ce que tu obtiens

- <projet> démarre → lit `CLAUDE.md` → importe `@claude-config/CLAUDE.md`
...
Tu demandes "déploie <projet> en prod et envoie un mail de release
à mes beta testeurs".
...
4. Il utilise l'**outil natif Bash** pour vérifier localement qu'un
   test de smoke passe.
5. Le tout est encapsulé dans un **skill** `/release-<projet>` qui
