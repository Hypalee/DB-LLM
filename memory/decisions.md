...
- **Contexte** : travailler avec Claude Code CLI sur plusieurs projets
  (Brain + à venir), besoin de continuité entre les sessions.
- **Options considérées** :
...
- **Statut** : décision **revue le 2026-04-26** — voir entrée
  "Découplage Brain ↔ DB-LLM" plus bas. Le pattern submodule a été
  abandonné pour Brain. La doc associée (`INTEGRATION.md`,
  `WORKFLOW.md`, chapitres `learning/`) est conservée comme trace
  pédagogique du pattern, pas comme prescription courante.

---

### 2026-04-26 — Découplage Brain ↔ DB-LLM
- **Contexte** : Brain a son propre cycle de release (bêta dans 6
  semaines), Hypalee est solo. L'architecture initiale (claude-config
  importé dans Brain via submodule + `@imports`) a montré une friction
...
- **Options considérées** :
  ...
  2. Découpler les deux repos. Brain devient auto-portant, DB-LLM
...
- **Décision** : option 2. Découplage acté côté Brain dans la PR
  Hypalee/Brain#47. Côté DB-LLM, ménage textuel à faire (cette
  session) en préservant la trace pédagogique du pattern abandonné.
- **Conséquences** :
  - Brain n'a plus de submodule `claude-config/`. Son `CLAUDE.md` est
    auto-portant.
  ...
  - Les chapitres `learning/` et `INTEGRATION.md` / `WORKFLOW.md` qui
    ...
    Brain — réévaluer si un 2e projet émerge".
