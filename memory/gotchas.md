...
- **Contexte** : <projet>, Better Auth + plugin 2FA TOTP.
...
- **Contexte** : <projet>, Vercel + domaine custom `<domaine.tld>`.
...
- **Fix** : poser `NEXT_PUBLIC_APP_URL=https://<domaine.tld>` et
  `BETTER_AUTH_URL=https://<domaine.tld>` **exactement**. Pas de `/`
  final, pas de `www.`.
